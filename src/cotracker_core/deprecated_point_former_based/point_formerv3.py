...
class Point(Dict):
    ...
    def serialization(self, order="z", depth=None, shuffle_orders=False):
        ...
        assert depth <= 16, f"depth is {depth}, {self.grid_coord.max()}"
...
class Embedding(PointModule):
    def __init__(
        ...
        # here use kernel 1x1x1, i.e., Linear Transformation on each individual point
...
class DefaultClassifier(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        #torch.scatter.segment_csr performs average pooling over each point cloud (separated by offset)
...
if __name__ == "__main__":
    backbone = PointTransformerV3(cls_mode=True, enable_flash=False)
    classifier = DefaultClassifier(backbone, num_classes=2, backbone_embed_dim=512)
    device = 'cuda:6'

    model = classifier.to(device)
    data_dict = {
        "feat": torch.randn(1000, 6).to(device),
        "coord": torch.rand(1000, 3).to(device) * 64,
        "grid_size": 0.01,
        "offset": torch.tensor([30, 60, 1000]).to(device)
    }

    cls_logits = model(data_dict)
