...
class BufferSOCTrainer(SOCTrainer):
    ...
    def recompute_buffer(self, chunk_id):
        ...
        for batch_start in range(0, len(local_indices), processing_batch_size):
            ...
            batch_prompts = [datamodule.train_dataset[i] for i in batch_indices]
            ...
            # Process this batch
            prompts_dict = {'text': [p['text'] for p in batch_prompts]}
            ...
            collected_data = self.collect_data(prompts_dict, batch_start)
        ...
        print(f"GPU {local_rank}: Buffer updated for chunk {chunk_id} with {len(self.accumulator['prompts'])} prompts in {len(self.accumulator['trajectories'])} batches")
