...
from trainers.soc_trainer import SOCTrainer
from trainers.buffer_soc_trainer import BufferSOCTrainer
...
class BufferAMTrainer(BufferSOCTrainer, AMTrainer):
    ...
    def __init__(self, config):
        ...
        self.buffer_variables = [
            'adjoint_states', 'trajectories', 'rewards', 'prompts', 'prompt_embeds',
            'negative_prompt_embeds', 'random_noises', 'noise_preds', 'noise_preds_init'
        ]
        self.buffer = {}
    ...
    def collect_data(self, prompts, batch_start):
        ...
        _, all_random_noises, all_noise_preds, all_x_t, prompt_embeds, negative_prompt_embeds = self.soc_pipeline(
        ...
        all_t = self.time_steps
        self.switch_to_eval()

        if self.config.cfg_adjoint:
            prompt_embeds_adj_computation = torch.cat([negative_prompt_embeds, prompt_embeds], dim=0)
        else:
            prompt_embeds_adj_computation = prompt_embeds

        adjoint_states, reward_values, all_noise_pred_init = self.compute_adjoints(
            all_x_t,
            all_t.to(torch.int),
            prompt_embeds_adj_computation,
            prompts,
            eta=self.config.eta,
    ...
    def control_difference_from_buffer(
        ...
        _, _, prev_sample_diff_from_buffer, std_dev_t, _ = self.soc_pipeline.scheduler.step(
            noise_pred_eval, 
            noise_pred_buffer, 
    ...
    def stepwise_log_importance_weights(self, control_difference, noise):
        # Integrated control norms - 0.5 ||u_1(x_t,t) - u_2(x_t,t)||² dt  [batchsize, timesteps, 4, 64, 64] -> [batchsize, timesteps]
        deterministic_path_weights = -0.5 * torch.sum(
            control_difference ** 2,
            dim=[2, 3, 4]
        )

        # Stochastic costs <u_1(x_t,t) - u_2(x_t,t), dB_t>  [batchsize, timesteps, 4, 64, 64] -> [batchsize, timesteps]
    ...
    def compute_training_loss(
        ...
        **kwargs,
        ...
        """
        prompt_embeds, negative_prompt_embeds, all_x_t, all_t, prompts, adjoint_states, reward_values, noise_preds, noise_preds_init, random_noises = batch
    ...
    def generate_trajectories_train(self, batch, batch_idx):
        ...
        prompt_embeds = self.buffer['prompt_embeds'][batch_idx_within_chunk]
        negative_prompt_embeds = self.buffer['negative_prompt_embeds'][batch_idx_within_chunk]
        x_t = self.buffer['trajectories'][batch_idx_within_chunk]
        prompts = self.buffer['prompts'][batch_idx_within_chunk]
        ...
        return prompt_embeds, negative_prompt_embeds, x_t, t, prompts, adjoint_states, reward_values, noise_preds, noise_preds_init, random_noises
